...
export default function CasesSection() {
  ...
  const filters: (CaseCategory | "Все")[] =
    presentCategories.length > 1 ? ["Все", ...presentCategories] : [];
  const [active, setActive] = useState<CaseCategory | "Все">("Все");

  const visible =
    active === "Все" ? cases : cases.filter((c) => c.category === active);

  return (
    <section id="cases" className="scroll-mt-24 py-24 lg:py-32">
      <div className="mx-auto max-w-6xl px-6 lg:px-8">
        <Reveal>
          <div className="label-uppercase text-primary">Кейсы</div>
